...
class PositionalEmbedding(nn.Module):
    def __init__(self, max_seq_len: int, features: int):
        ...
        self.positions = torch.arange(0, max_seq_len).cpu()
...
class Decoder(nn.Module):
    ...
    def causal_mask(x):
        n = x.shape[1]
        return (1. - torch.tril(torch.ones(n, n).cpu())) * -1e9
...
if __name__ == '__main__':
    ...
    encoder_inputs_ = torch.tensor([[1, 5, 6, 4, 3, 2], [1, 3, 7, 4, 2, 0]]).cpu()
    decoder_inputs_ = torch.tensor([[1, 3, 4, 6, 5], [1, 4, 7, 3, 2]]).cpu()
